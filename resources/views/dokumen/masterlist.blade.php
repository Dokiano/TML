...
{{-- -masih percobaan untuk loop dibawah ini --}}
@php
  $u = auth()->user();
...
@php
  ...
  $ckrNames    = [ 'QA CKR','PRODUCTION CKR','MTC MEC DAN ELC CKR','MTC UTILIYY CKR','HR GA CKR','SAFETY CKR','SUPPLY CHAIN CKR','WAREHOUSE CKR','MANUFACTURING','LAB','ENGINEERING', 
];
  ...
  // ini dari controller kalau pernah dikirim; kalau tidak ada juga aman
  $userDivisiId = $userDivisiId ?? null;

  $upper = fn($a)=>array_map('strtoupper',$a);
  $allowByName = function ($divName) use ($ckrNames,$commonNames,$upper) {
      $allowed = array_unique(array_merge($upper($ckrNames), $upper($commonNames)));
...
<section class="section container-fluid">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="page-title text-center mb-4">Master List Dokumen – Cikarang</h4>
  ...
  <div class="row g-4 justify-content-center">
    @forelse($divisis as $divisi)
      @php
        $tampil =/* $allowById($divisi->id) && */$allowByName($divisi->nama_divisi);
      @endphp
      ...
      <div class="col-auto">
        <a href="{{ route('dok.master.ckr.divisi', ['id'=>$divisi->id]) }}" class="text-decoration-none text-dark">
